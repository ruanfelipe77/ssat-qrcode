    ...
    if (modalEl) {
      ...
      $(document).on("click", ".edit-order", async function () {
        ...
        try {
          ...
          const orderProducts =
            typeof orderProductsRes === "string"
              ? JSON.parse(orderProductsRes)
              : orderProductsRes;
          const availableProducts =
            typeof availableRes === "string"
              ? JSON.parse(availableRes)
              : availableRes;
              ...
              <div class="product-card p-2 bg-white border rounded shadow-sm h-100" data-type="${(
                ...
                <div class="form-check h-100">
                  <input class="form-check-input product-select" type="checkbox" value="${
                    p.id
                  }" id="product_edit_${p.id}" name="products[]" ${checked}>
                  <label class="form-check-label w-100 h-100 d-flex flex-column" for="product_edit_${
                    p.id
                  }">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                      ...
                      <i class="fas ${
                        checked ? "fa-check-circle" : "fa-plus-circle"
                      } text-success"></i>
                    </div>
                    <div class="flex-grow-1">
                      <div class="d-flex flex-wrap gap-1 mb-1">
                        <span class="badge bg-dark" style="font-size:0.7rem;"><i class="fas fa-hashtag me-1"></i>${
                          p.serial_number || ""
                        }</span>
                        <span class="badge bg-warning text-dark" style="font-size:0.7rem;"><i class="fas fa-shield-alt me-1"></i>${
                          p.warranty || ""
                        }</span>
                      </div>
                    </div>
        ...
        `;
          ...
          if (typeof updateProductCardStyles === "function")
          ...
          Swal.fire({
            title: "Erro",
            text: "Não foi possível carregar o pedido para edição.",
            icon: "error",
          });
        }
      });
