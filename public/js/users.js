$(document).ready(function () {
  ...
  const table = $("#users-table").DataTable({
    ...
    initComplete: function () {
      $("#users-table").addClass("initialized");
        ...
        $.post(
          ...
          function (res) {
            ...
            if (r.success) {
              ...
              Swal.fire({
                title: "Erro",
                text: r.message || "Falha ao excluir",
                icon: "error",
              });
            }
          },
  ...
  $("#userForm").on("submit", function (e) {
    ...
    const payload =
      $(this).serialize() +
      `&action=${encodeURIComponent(action)}&active=${activeChecked}`;
    ...
    $.post("src/controllers/UserController.php", payload, function (res) {
      ...
      if (r && r.success) {
        ...
        Swal.fire({
          title: "Sucesso",
          text: action === "add" ? "Usuário criado" : "Usuário atualizado",
          icon: "success",
        }).then(() => location.reload());
      } else {
        Swal.fire({
          title: "Erro",
          text: r && r.message ? r.message : "Falha ao salvar usuário",
          icon: "error",
        });
      }
    });
